...
4. **Keys** — Remove `force_render` on every console key; keep `bump_fast_timer`. `reset_caret_blink` dirties only if caret was off.
5. **Caret schedule** — Drop perpetual `caret_urgency` → `TICK_FAST`; schedule blink wakes on ~530 ms; idle at `TICK_IDLE` when not dirty.
6. **Incremental flat lines** — On volatile-only active updates, patch Console `flat_lines` tail; leave other views dirty for later switch.
