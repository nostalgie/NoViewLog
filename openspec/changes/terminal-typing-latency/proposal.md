...
Holding a key in the Console feels several times slower than a native Ubuntu terminal. Stdin already writes to the PTY immediately; the lag is display — echo waits on a ~33 ms UI tick, then pays for a full flat-line rebuild and full-frame fontdue paint. A slow Console makes the product feel unusable.
...
- Update Console flat lines incrementally for volatile VT tail changes instead of full scrollback rebuilds.
...
- `engine/console-latency`: Console typing/echo display latency and incremental volatile updates for the live screen.
