...
use crate::file_index::{decode_lossy_line, strip_bom};
...
/// Returns the next byte position to continue from (and whether the scan is
/// finished — file end or the offset cap reached).
...
pub(crate) fn scan_match_chunk_with_cap(
    ...
    while pos < end_byte {
        ...
        match reader.read_until(b'\n', &mut raw) {
            ...
            Ok(n) => {
                pos += n as u64;
                let decoded = decode_lossy_line(&raw);
                ...
                let visible = filters.is_visible_text(line)
                    && (severity == SeverityFilter::All
                        || severity.allows(detect_level(line)));
                if visible {
...
mod tests {
    ...
    fn scan_stops_at_offset_cap() {
        ...
        let mut offsets = Vec::new();
        let (pos, done) = scan_match_chunk_with_cap(
            &mut file,
            size,
            0,
            size,
