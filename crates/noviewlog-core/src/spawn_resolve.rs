...
fn default_interactive_shell(shell: ShellPreference) -> String {
    #[cfg(windows)]
    {
        match shell {
...
mod tests {
    ...
    fn resolve_interactive_shell_honors_windows_preference() {
        ...
        {
            let (exe, _, _) =
                resolve_interactive_shell(&launch, ShellPreference::Cmd).unwrap();
            ...
            let (auto, _, _) =
                resolve_interactive_shell(&launch, ShellPreference::Auto).unwrap();
            assert!(auto == "pwsh.exe" || auto == "powershell.exe", "auto={auto}");
        }
        #[cfg(not(windows))]
        {
            let (exe, _, _) =
